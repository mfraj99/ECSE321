...
public class PetShelterIntegrationTests {
	...
	/*
	 * FR1 tests No need for testing of missing inputs, http throws errors
	 * automatically
	 */
	...
	public void AtestCreateAppUser() {
		try {
			response = send("POST", APP_URL, "/appuser/" + usernameUser,
					"password=" + passwordUser + "&appUserRole=" + PersonRole.ADOPTER);
...
//	}
	...
	public void GtestCreateUserProfile() {
		CtestLogInAppUser();
		response = send("POST", APP_URL, "/userprofile/" + usernameUser,
				"address=" + address + "&hasExperienceWithPets=" + true + "&numberOfPetsCurrentlyOwned"
						+ numberOfPetsCurrentlyOwned + "&typeOfLivingAccommodation" + typeOfLivingAccommodation);
	}
